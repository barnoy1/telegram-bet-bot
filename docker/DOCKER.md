...
Set in `docker/.env` before running `docker compose up`:
...
# 2. Update docker/.env
echo "OLLAMA_MODEL=llama2:7b" >> docker/.env
...
Database files are stored in `../data/` directory on host machine (relative to docker/ folder), so they survive container restarts.
...
- `docker/Dockerfile` - Multi-stage build optimizing image size
- `docker/docker-compose.yml` - Services: Ollama, Bot, Network, Volumes
